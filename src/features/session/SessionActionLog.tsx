...
export function SessionActionLog() {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      SESSION_PANEL_COLLAPSED_STORAGE_KEY,
      collapsed ? "1" : "0",
    )
  }, [collapsed])
    ...
    <>
      <div className="flex items-center justify-between border-b border-border px-3 py-2">
        ...
        {entries.length ? (
          <button
            type="button"
            onClick={onClear}
            className="rounded px-2 py-1 text-[10px] font-medium text-textMuted transition-colors hover:bg-bg-subtle hover:text-textH"
          >
            Limpar
          </button>
        ) : null}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        {entries.length ? (
          ...
          <div className="rounded-xl border border-dashed border-border px-4 py-8 text-center text-xs leading-5 text-textMuted">
            As rolagens da sessão aparecerão aqui.
          </div>
        )}
      </div>
    </>
...
function DiceRollEntry({
  ...
  roll: SessionDiceRollResult
  characterName?: string
}) {
  const naturalClass = roll.natural === 20
    ? "text-success"
    : roll.natural === 1
      ? "text-danger"
      : "text-textH"
  ...
  return (
    <article className="rounded-lg border border-border bg-bg px-3 py-2.5">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className={`shrink-0 text-2xl font-black ${naturalClass}`}>
          {roll.total}
        </div>
      </div>
