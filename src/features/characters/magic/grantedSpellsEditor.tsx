      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[11px] leading-4 text-textMuted">
            Escolha uma magia do catálogo e defina se ela usa o recurso da origem ou espaços normais de magia.
          </p>
        </div>
          ...
          {grants.map((grant, index) => {
            ...
            return (
              <div
                ...
                className="grid gap-3 rounded-lg border border-border bg-bg p-3"
              >
                <div className="flex items-start justify-between gap-3">
                ...
                <div className="grid gap-2 md:grid-cols-2">
                  <label className="grid gap-1">
                    ...
                    <Select
                      ...
                      onChange={(event) =>
                        updateGrant(index, {
                          castingMode: event.target
                            .value as SpellGrantCastingMode,
                        })
                      }
                    >
                      <option value="source">
                      ...
                      <Input
                        ...
                        onChange={(event) => {
                          ...
                          updateGrant(index, {
                            usage: {
                              max,
                              used: Math.min(usage?.used ?? 0, max),
                ...
                {variant === "ability" && castingMode === "source" ? (
                  <div className="rounded-lg bg-bg-subtle px-3 py-2 text-[11px] text-textMuted">
                    {abilityHasUsage
                      ? "A magia usa o contador de usos da própria habilidade."
                      : "A magia pode ser conjurada pela habilidade sem um contador configurado."}
                  </div>
                ) : null}
  ...
  const filteredSpells = useMemo(() => {
    ...
    return spells
      ...
      .filter((spell) => {
        const matchesSearch =
          !normalizedSearch ||
          spell.name.toLocaleLowerCase().includes(normalizedSearch) ||
          spell.displayName
            ?.toLocaleLowerCase()
        ...
        <div className="flex items-start justify-between gap-3 border-b border-border p-4">
          <div>
            ...
            <p className="mt-1 text-xs text-textMuted">
              Magias oficiais e homebrew disponíveis no módulo de Magia.
            </p>
          </div>
          ...
          {filteredSpells.length > 0 ? (
            filteredSpells.map((spell) => (
              <button
                ...
                className="rounded-lg border border-border bg-bg p-3 text-left transition-colors hover:border-accentBorder hover:bg-accentBg"
              >
