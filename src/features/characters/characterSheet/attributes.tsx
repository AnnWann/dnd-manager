...
export function Attributes({ character, updateCharacter }: Props) {
  const { dispatchAttributeOperation } = useCharacterWorkspace()
    ...
    <section className="rounded-xl border border-border bg-bg p-3 shadow-theme-sm">
      <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h2 className="text-sm font-semibold text-textH">Atributos</h2>
      ...
      <div className="grid grid-cols-2 gap-2 xl:grid-cols-1">
        {ATTRIBUTE_KEYS.map((attribute) => {
          const baseScore = character.get("sheet").attributes[attribute]
          const racialBonus = character.get("sheet").race.attributeBonus?.[attribute] ?? 0
          ...
          return (
            <div key={attribute} className="grid min-w-0 gap-2 rounded-lg border border-border bg-bg-subtle p-3">
              <div className="flex min-w-0 items-center justify-between gap-2">
                ...
                <div className="flex shrink-0 items-baseline gap-1 text-right">
                  ...
                  {showRawValues ? (
                    <span className="px-1 text-xl font-bold text-textH">
              ...
              <label className="grid min-w-0 gap-1 text-center">
                <span className="text-[10px] uppercase tracking-wide text-textMuted">{showRawValues ? "Bruto" : "Final"}</span>
                <Input
                  type="number"
                  inputMode="numeric"
                  ...
                  min={1}
                  max={30}
                  onChange={(event) => updateDisplayedScore(Number(event.target.value))}
                />
              </label>
              ...
              {!showRawValues ? (
                <div className="min-h-4 break-words text-center text-[10px] leading-4 text-textMuted">
                  Base {baseScore}
                  {racialBonus !== 0 ? ` • Raça ${formatSigned(racialBonus)}` : ""}
                  {asiBonus !== 0 ? ` • ASI ${formatSigned(asiBonus)}` : ""}
                  {otherBonus !== 0 ? ` • Outros ${formatSigned(otherBonus)}` : ""}
                </div>
              ) : null}
            </div>
          )
      ...
      {asis.length ? (
        <div className="mt-3 border-t border-border pt-3">
          <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-textMuted">ASI e talentos</div>
          <div className="grid gap-2">
            {asis.toSorted((left, right) => left.classLevel - right.classLevel).map((asi) => (
              <div key={asi.id} className="flex items-start justify-between gap-3 rounded-lg border border-border bg-bg-subtle px-3 py-2 text-xs">
                <div className="min-w-0">
                  <div className="font-medium text-textH">{formatAsiLabel(asi)}</div>
                  <div className="mt-0.5 text-textMuted">Nível {asi.classLevel} de {asi.className}</div>
                </div>
                <div className="shrink-0 text-right font-semibold text-textH">{formatAsiIncrease(asi)}</div>
              </div>
            ))}
          </div>
        </div>
...
function formatAsiIncrease(asi: CharacterAsi): string {
  const parts = ATTRIBUTE_KEYS
    ...
    .map((attribute) => `${formatSigned(asi.increases[attribute] ?? 0)} ${attributeLabel(attribute)}`)
