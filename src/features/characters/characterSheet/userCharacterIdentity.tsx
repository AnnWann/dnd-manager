...
export function UserCharacterIdentity({ character, updateCharacter }: Props) {
  ...
  const classDescription =
    classes.length > 0
      ? classes
          ...
          .join(" / ")
      : "Sem classe"
  const canEditClasses = isEditing && character.get("sheet").type === "pc"
  ...
  return (
    <section className="rounded-xl border border-border bg-bg p-4 shadow-theme-sm">
        ...
        <div className="grid grid-cols-[1fr_auto] gap-3">
          <button
            ...
            title={
              canEditClasses
                ? "Editar classes"
                : "Ative o modo de edição para alterar classes"
            }
            onClick={() => {
      ...
      <ClassEditorModal
        open={classEditorOpen && canEditClasses}
        character={character}
...
function ClassEditorModal({
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="user-class-editor-title"
      onMouseDown={(event) => {
