...
import { canPaySpellResourceCost, getSpellResourceCurrent, spellResourceLabel, spendSpellResourceCost } from "../../../models/magic/spells/spellResourceCost"
...
type CastingResource = "slot" | "ability"
...
export function MinimalMagicActions({ character, updateCharacter }: Props) {
  ...
  const visibleSpells = allSpells
    ...
    .sort((left, right) => left.spell.slotLevel - right.spell.slotLevel || spellName(left.spell).localeCompare(spellName(right.spell), "pt-BR"))
  ...
  const hasChannelDivinity = Boolean(channelDivinity && channelDivinity.max > 0)
  ...
  const hasMagicResources = hasSlots || hasPactSlots || hasSorceryPoints || hasChannelDivinity || hasKi || abilityChargeResources.length > 0
  ...
  const selectedAlternateCost = selected?.spell.resourceCost
  const alternateCurrent = selectedAlternateCost
    ? getSpellResourceCurrent(character, selectedAlternateCost.resource)
    : 0
  const alternateAvailable = selectedAlternateCost
    ? canPaySpellResourceCost(character, selectedAlternateCost)
    : false
  const asksCastLevel = Boolean(
    selected &&
      !selectedAlternateCost &&
  ...
  function openSpell(entry: MinimalSpellEntry) {
    ...
    setCastingResource(entry.sourceUsageSource && (entry.sourceUsageRemaining ?? 0) > 0 ? "ability" : "slot")
  }
  ...
  function executeSelectedCast() {
    ...
    const canUseAbilityCharge = Boolean(selected.sourceUsageSource && selected.sourceUsageMaximum !== undefined)
    const useAbilityCharge = selected.sourceCastingMode === "source" || (canUseAbilityCharge && castingResource === "ability")
    ...
    if (spell.resourceCost) {
      if (!canPaySpellResourceCost(character, spell.resourceCost)) {
        setError(`Não há ${spellResourceLabel(spell.resourceCost.resource)} suficiente para conjurar esta magia.`)
        setConfirmConcentrationReplacement(false)
        return
      }
      updateCharacter(character.get("id"), (current) => {
        let next = spendSpellResourceCost(current, spell.resourceCost!)
        if (spell.concentration) next = beginSpellConcentration(next, spell)
        return next
      })
      finishCast()
    ...
    if (spell.slotLevel === 0) {
      if (spell.concentration) {
        updateCharacter(character.get("id"), (current) => beginSpellConcentration(current, spell))
      }
      finishCast()
      return
    }
    ...
    updateCharacter(character.get("id"), (current) => {
      let next = choice.pool === "pact" ? current.spendPactSlot() : current.spendSpellSlot(choice.level)
      if (spell.concentration) next = beginSpellConcentration(next, spell)
      return next
    })
    finishCast()
  ...
  const useDisabled = selected
    ? selected.sourceCastingMode === "source"
      ? selected.sourceUsageRemaining === 0
      : castingResource === "ability" && selected.sourceUsageSource
        ? selected.sourceUsageRemaining === 0
        : selected.spell.resourceCost
          ? !alternateAvailable
        ...
        <>
          <div className="flex flex-wrap items-end justify-between gap-2">
            <div>
              ...
              <p className="mt-1 text-[10px] leading-4 text-textMuted">Magias conhecidas, preparadas ou concedidas por habilidades e equipamentos.</p>
            </div>
            {availableLevels.length > 1 ? (
              <label className="grid gap-1 text-[10px] text-textMuted">
                Nível
                <select className="h-8 rounded-lg border border-border bg-bg px-2 text-xs text-textH" value={levelFilter} onChange={(event) => setLevelFilter(event.target.value === "all" ? "all" : Number(event.target.value))}>
                  <option value="all">Todos</option>
                  {availableLevels.map((level) => <option key={level} value={level}>{level === 0 ? "Truques" : `Nível ${level}`}</option>)}
                </select>
              </label>
            ) : null}
          </div>
          ...
          {visibleSpells.length ? (
            <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-4">
              {visibleSpells.map((entry) => (
                <button key={entry.key} type="button" onClick={() => openSpell(entry)} className="min-h-14 rounded-lg border border-border bg-bg-subtle px-3 py-2 text-left transition-colors hover:border-accentBorder hover:bg-accentBg">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex min-w-0 gap-1.5 text-[10px] text-textMuted">
                        <span className="shrink-0">{entry.spell.slotLevel === 0 ? "Truque" : `N${entry.spell.slotLevel}`}</span><span>•</span><span className="truncate">{sourceLabel(entry.source)}</span>
                      </div>
                      {entry.spell.resourceCost ? (
                        <div className="mt-1 text-[10px] font-semibold text-accent">{entry.spell.resourceCost.amount} {spellResourceLabel(entry.spell.resourceCost.resource)}</div>
                      ) : null}
                    </div>
                    {entry.sourceUsageMaximum !== undefined ? <span className="shrink-0 rounded-md border border-accentBorder bg-accentBg px-2 py-1 text-[10px] font-semibold text-textH">{entry.sourceUsageRemaining ?? 0}/{entry.sourceUsageMaximum} usos</span> : null}
                  </div>
                </button>
              ))}
            </div>
          ) : <div className="mt-2 rounded-lg border border-dashed border-border bg-bg-subtle px-3 py-3 text-xs text-textMuted">Nenhuma magia disponível nesta categoria e nível.</div>}
        </>
      ) : null}
      ...
      {hasMagicResources ? (
        <div className={allSpells.length ? "mt-4 border-t border-border pt-3" : ""}>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            {pactSlots && pactSlots.max > 0 ? <ResourcePill label={`Pacto N${pactSlots.level}`} current={pactSlots.current} max={pactSlots.max} accent onDecrease={() => updateCharacter(character.get("id"), spendPactSlot)} onIncrease={() => updateCharacter(character.get("id"), restorePactSlot)} /> : null}
            ...
            {abilityChargeResources.map((resource) => <ResourcePill key={`ability-charge:${resource.label}`} label={resource.label} current={resource.current} max={resource.max} accent />)}
          </div>
        </div>
      ) : null}
        ...
        <Modal title={spellName(selected.spell)} onClose={() => setSelected(null)} className="max-w-xl">
          <div className="grid gap-3">
            <div className="flex flex-wrap gap-2 text-[10px] font-semibold uppercase tracking-wide text-textMuted">
              <span>{selected.spell.slotLevel === 0 ? "Truque" : `Nível ${selected.spell.slotLevel}`}</span><span>• {actionFilterLabel(normalizeCastingTime(selected.spell))}</span><span>• {sourceLabel(selected.source)}</span>{selected.spell.concentration ? <span>• Concentração</span> : null}
            </div>
            <p className="max-h-56 overflow-y-auto whitespace-pre-wrap text-sm leading-6 text-text">{selected.spell.description?.trim() || "Sem descrição."}</p>
            {selected.spell.higherLevelText?.trim() ? <div className="rounded-lg border border-border bg-bg-subtle px-3 py-2 text-xs leading-5 text-text"><div className="font-semibold text-textH">Em níveis superiores</div><div className="mt-1 whitespace-pre-wrap">{selected.spell.higherLevelText}</div></div> : null}

            {selected.spell.resourceCost && selected.sourceCastingMode !== "source" ? (
              <div className="rounded-lg border border-accentBorder bg-accentBg px-3 py-2 text-xs text-text">
                <div className="font-semibold text-textH">Custo de conjuração</div>
                <div className="mt-1">Consome <strong>{selected.spell.resourceCost.amount} {spellResourceLabel(selected.spell.resourceCost.resource)}</strong> em vez de espaço de magia. Disponível: {alternateCurrent}.</div>
              </div>
            ) : null}
            ...
            {selected.sourceUsageSource && selected.sourceCastingMode === "slots" && !selected.spell.resourceCost ? (
              <label className="grid gap-1 text-xs text-textMuted">Recurso para conjurar
                <select className="h-10 rounded-lg border border-border bg-bg px-3 text-sm text-textH" value={castingResource} onChange={(event) => setCastingResource(event.target.value as CastingResource)}>
                  <option value="ability" disabled={(selected.sourceUsageRemaining ?? 0) <= 0}>{selected.sourceUsageLabel || sourceLabel(selected.source)} — {selected.sourceUsageRemaining ?? 0}/{selected.sourceUsageMaximum ?? 0} usos</option>
                  <option value="slot" disabled={selected.spell.slotLevel > 0 && slotChoices.length === 0}>Espaço de magia</option>
                </select>
              </label>
            ) : null}

            {asksCastLevel && castingResource === "slot" ? <label className="grid gap-1 text-xs text-textMuted">Nível de conjuração<select className="h-10 rounded-lg border border-border bg-bg px-3 text-sm text-textH" value={castLevel ?? ""} onChange={(event) => setCastLevel(Number(event.target.value))}>{Array.from(new Set(slotChoices.map((choice) => choice.level))).map((level) => <option key={level} value={level}>Nível {level}</option>)}</select></label> : null}
            {error ? <div className="rounded-lg border border-danger bg-dangerBg px-3 py-2 text-xs text-danger">{error}</div> : null}
            <div className="flex justify-end border-t border-border pt-3"><Button variant="primary" disabled={useDisabled} onClick={castSelected}>Usar</Button></div>
          </div>
        </Modal>
      ) : null}

      {selected && confirmConcentrationReplacement ? (
        <Modal title="Substituir concentração?" onClose={() => setConfirmConcentrationReplacement(false)} className="max-w-md">
          <div className="grid gap-3">
            <p className="text-sm leading-6 text-text">O personagem já está concentrando{currentConcentration?.source ? ` em ${currentConcentration.source}` : " em outra magia"}. Conjurar <strong>{spellName(selected.spell)}</strong> encerrará a concentração anterior.</p>
            <p className="text-xs leading-5 text-textMuted">O recurso da nova magia só será gasto depois da confirmação.</p>
            <div className="flex flex-wrap justify-end gap-2 border-t border-border pt-3"><Button variant="secondary" onClick={() => setConfirmConcentrationReplacement(false)}>Cancelar</Button><Button variant="primary" onClick={executeSelectedCast}>Conjurar e substituir</Button></div>
          </div>
        </Modal>
      ) : null}
    </section>
...
function buildAvailableSpells(character: CharacterTemplate, getSpellByIndex: (index: string) => Spell | undefined): MinimalSpellEntry[] {
  ...
  for (const known of character.get("magic")?.spells.knownSpells ?? []) {
    ...
    entries.push({ key: `known:${known.source.type}:${known.source.sourceId}:${spell.index}`, spell, source: known.source, sourceCastingMode: "slots" })
  }
  for (const grant of getCharacterGrantedSpells(character)) {
    ...
    const remaining = grant.usage && maximum !== undefined ? Math.max(0, maximum - grant.usage.used) : undefined
    entries.push({ key: grant.key, spell, source: grant.source, sourceCastingMode: grant.castingMode === "known" ? "slots" : "source", sourceUsageRemaining: remaining, sourceUsageMaximum: maximum, sourceUsageLabel: grant.usageSource ? grant.source.name || "Carga de habilidade" : undefined, sourceUsageSource: grant.usageSource })
  }
  return entries
}

function isAlwaysAvailableSpell(spell: Spell, source: SpellSource, classes: CharacterClassInterface[]): boolean {
...
function getSlotChoices(character: CharacterTemplate, spell: Spell): SlotChoice[] {
  ...
  if (pact && pact.current > 0 && pact.level >= spell.slotLevel) choices.push({ level: pact.level as MagicCircleLevel, pool: "pact" })
  return choices.sort((left, right) => left.level - right.level || (left.pool === "normal" ? -1 : 1))
}

function ResourcePill({ label, current, max, accent = false, onDecrease, onIncrease }: { label: string; current: number; max: number; accent?: boolean; onDecrease?: () => void; onIncrease?: () => void }) {
  const editable = Boolean(onDecrease || onIncrease)
  return <div className={cn("flex items-center gap-2 rounded-lg border px-2.5 py-2 text-xs", accent ? "border-accentBorder bg-accentBg" : "border-border bg-bg-subtle")}><div className="min-w-0"><span className="font-semibold text-textH">{label}</span>{" "}<span className="text-textMuted">{current}/{max}</span></div>{editable ? <div className="ml-auto flex items-center gap-1"><button type="button" aria-label={`Consumir ${label}`} title={`Consumir ${label}`} disabled={!onDecrease || current <= 0} onClick={onDecrease} className="grid h-6 w-6 place-items-center rounded-md border border-border bg-bg text-sm font-semibold text-textH transition-colors hover:border-accentBorder hover:bg-accentBg disabled:cursor-not-allowed disabled:opacity-35">−</button><button type="button" aria-label={`Recuperar ${label}`} title={`Recuperar ${label}`} disabled={!onIncrease || current >= max} onClick={onIncrease} className="grid h-6 w-6 place-items-center rounded-md border border-border bg-bg text-sm font-semibold text-textH transition-colors hover:border-accentBorder hover:bg-accentBg disabled:cursor-not-allowed disabled:opacity-35">+</button></div> : null}</div>
}

function spellName(spell: Spell): string { return spell.displayName || spell.name }
function sourceLabel(source: SpellSource): string {
  if (source.type === "class") return CLASS_NAMES[source.name as keyof typeof CLASS_NAMES] ?? source.name ?? "Classe"
