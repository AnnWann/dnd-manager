...
export function CharacterCreationEquipmentChoicesStable({ onChange }: Props) {
  ...
  useEffect(() => {
    ...
    const findTarget = () => {
      if (disposed) return
      if (target?.anchor.isConnected && target.source.isConnected) return
      ...
      const heading = Array.from(document.querySelectorAll<HTMLElement>("h2")).find(
        (entry) =>
          ...
          !entry.closest("[data-stable-class-equipment]") &&
      ...
      if (
        ...
        anchor.dataset.stableClassEquipmentAnchor !== "true"
      ) {
      ...
      source.dataset.stableClassEquipmentSource = "true"
      source.style.display = "none"
      setTarget((current) =>
        current?.anchor === anchor && current.className === className
          ? current
    ...
    return () => {
      ...
      document
        .querySelectorAll<HTMLElement>("[data-stable-class-equipment-source]")
        .forEach((source) => {
          source.style.display = ""
          delete source.dataset.stableClassEquipmentSource
        })
      document
        .querySelectorAll<HTMLElement>("[data-stable-class-equipment-anchor]")
...
function ClassEquipmentConfigurator({
  ...
  const items = useMemo(() => {
    ...
    return selectedSpecs.map((spec) => {
      const resolved = genericWeapons[spec.id]
      const item = resolved ?? createStartingInventoryItem(spec)
      return normalizeStandardItem({
        ...item,
        id: resolved?.id ?? `starting-${className}-${spec.id}`,
        notes: mergeNotes(
          item.notes,
  ...
  return (
    <section
      data-stable-class-equipment="true"
