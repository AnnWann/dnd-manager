...
export function CustomAbilityLibraryEditor({
  ...
  async function copyTemplate() {
    ...
    try {
      ...
      setJsonFeedback({
        kind: 'error',
        message: 'O navegador não permitiu copiar automaticamente. O modelo foi aberto para cópia manual.',
      })
    }
  }
  ...
  function importAbilities() {
    ...
    try {
      ...
      if (duplicateIds.length) {
        throw new Error(
          `O JSON possui IDs repetidos: ${Array.from(new Set(duplicateIds.map((entry) => entry.id))).join(', ')}.`,
        )
      }
      ...
      if (importMode === 'replace') {
        ...
        if (
          collisions.length &&
          !window.confirm(
            `${collisions.length} habilidade(s) já existem e serão substituídas pelos dados importados. Continuar?`,
          )
        ) return
        nextAbilities = [
      ...
      setJsonFeedback({
        kind: 'error',
        message: error instanceof Error ? error.message : 'Não foi possível importar o JSON.',
      })
    }
  }
  ...
  return (
    <section>
      <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-text">
            Cadastre habilidades prontas. Regras gerais pertencem ao tipo; exceções de uma habilidade específica pertencem aqui no compêndio.
          </p>
        </div>
        {type ? (
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm text-textH hover:bg-accentBg"
            >
      ...
      {!draft.abilityTypes.length ? (
        <Empty>Crie primeiro um tipo de habilidade na aba Avançado.</Empty>
      ) : (
        <div className="grid gap-4 xl:grid-cols-[240px_280px_minmax(0,1fr)]">
              ...
              {draft.abilityTypes.map((entry, index) => (
                ...
                >
                  ...
                  <span className="mt-1 block text-[11px]">
                    {entry.predefinedAbilities?.length ?? 0} habilidade(s)
                  </span>
                </button>
              ))}
            </div>
          ...
          <aside className="rounded-xl border border-border p-3">
            <div className="flex items-center justify-between gap-2">
              <div>
                <h3 className="font-medium text-textH">Compêndio</h3>
                <p className="mt-1 text-xs text-text">Habilidades disponíveis aos jogadores.</p>
              </div>
              <button
                type="button"
                onClick={addAbility}
                className="rounded-lg border border-accent p-2 text-accent"
              ...
              {abilities.map((entry, index) => (
                <button
                  ...
                  className={`rounded-lg border px-3 py-2 text-left ${
                    index === abilityIndex ? 'border-accent bg-accentBg' : 'border-border'
                  }`}
                >
                  <div className="text-sm font-medium text-textH">
                    {presetTitle(type, entry)}
                  </div>
                  <div className="mt-1 truncate font-mono text-[11px] text-text">
                    {entry.id}
                  </div>
                  {entry.activation ? (
                    <div className="mt-1 text-[10px] text-accent">Ativação específica</div>
                  ) : null}
                </button>
              ))}
            ...
            {type && ability ? (
              <div className="grid gap-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <h3 className="font-semibold text-textH">Editar habilidade</h3>
                    <p className="mt-1 text-xs text-text">
                      Dados e exceções desta habilidade pertencem ao compêndio; o restante é herdado do tipo “{type.name}”.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={removeAbility}
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <Input
                    label="ID estável"
                    value={ability.id}
                    onChange={(id) => replaceAbility({ ...ability, id: slugify(id) })}
                  />
                  <Input
                    label="Observação do mestre"
                    value={ability.description ?? ''}
                    onChange={(description) =>
                      replaceAbility({ ...ability, description: description || undefined })
                    }
                  />
                </div>
                      ...
                      .map((field) => (
                        <PresetField
                          ...
                          onChange={(value) =>
                            replaceAbility({
                              ...ability,
                              values: { ...ability.values, [field.id]: value },
                            })
                          }
                        />
                      ))}
                  </div>
                  {!type.fields.some((field) => field.type !== 'formula') ? (
                    <p className="mt-3 text-sm text-text">Este tipo ainda não possui campos editáveis.</p>
                  ) : null}
                </section>
                ...
                <label className="flex items-center gap-2 text-sm text-textH">
                  <input
                    type="checkbox"
                    checked={Boolean(type.allowCustomCreation)}
                    onChange={(event) =>
                      replaceType({ ...type, allowCustomCreation: event.target.checked })
                    }
                  />
                  Permitir que o jogador crie habilidades fora da biblioteca
                </label>
              </div>
            ) : (
              <Empty>Selecione ou adicione uma habilidade.</Empty>
            )}
          </main>
        </div>
      )}
...
function JsonImportModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/65 p-3"
      role="dialog"
      aria-modal="true"
      onMouseDown={(event) => {
      ...
      <section className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-xl border border-border bg-bg-elevated shadow-2xl">
        <header className="flex items-start justify-between gap-3 border-b border-border p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-text">
                A propriedade <code>activation</code> é opcional. Quando omitida, a habilidade herda o tipo.
              </p>
            </div>
          </div>
          <button
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <section className="grid content-start gap-3">
              ...
              <button
                ...
                className="rounded-lg border border-border px-3 py-2 text-sm text-textH hover:bg-accentBg"
              >
          ...
          <fieldset className="mt-4 rounded-lg border border-border p-3">
            ...
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              <label className={`flex cursor-pointer items-start gap-2 rounded-lg border p-3 ${importMode === 'append' ? 'border-accent bg-accentBg' : 'border-border'}`}>
                ...
                <span>
                  <strong className="block text-sm text-textH">Adicionar e atualizar</strong>
                  <span className="mt-1 block text-xs text-text">Mantém habilidades atuais. IDs iguais são substituídos.</span>
                </span>
              </label>
              <label className={`flex cursor-pointer items-start gap-2 rounded-lg border p-3 ${importMode === 'replace' ? 'border-accent bg-accentBg' : 'border-border'}`}>
                ...
                <span>
                  <strong className="block text-sm text-textH">Substituir biblioteca</strong>
                  <span className="mt-1 block text-xs text-text">Remove todas as habilidades atuais deste tipo.</span>
                </span>
              </label>
            </div>
          </fieldset>
        ...
        <footer className="flex flex-wrap justify-end gap-2 border-t border-border p-4">
          ...
          <button
            ...
            className="rounded-lg border border-accent bg-accent px-4 py-2 text-sm font-medium text-accentText disabled:cursor-not-allowed disabled:opacity-50"
          >
...
function readImportedAbilities(
  ...
  if (
    isRecord(value) &&
    typeof value.abilityTypeId === 'string' &&
    value.abilityTypeId !== type.id
  ) {
    throw new Error(
      `Este JSON foi criado para o tipo “${value.abilityTypeId}”, mas o tipo selecionado é “${type.id}”.`,
    )
  }

  return source.map((entry, index) => normalizeImportedAbility(entry, type, index))
}

function normalizeImportedAbility(
  value: unknown,
  type: CustomAbilityTypeDefinition,
  index: number,
): CustomPredefinedAbilityDefinition {
  ...
  for (const field of allowedFields.values()) {
    if (
      ...
      throw new Error(
        `A habilidade “${id}” não informou o campo obrigatório “${field.id}”.`,
      )
    }
  ...
  return {
    ...
    description:
      typeof value.description === 'string' && value.description.trim()
        ...
        : undefined,
    values,
    activation: isRecord(value.activation)
      ? structuredClone(value.activation) as unknown as CustomAbilityActivationDefinition
      : undefined,
    acquisition: isRecord(value.acquisition)
      ? structuredClone(value.acquisition) as unknown as Partial<CustomAbilityAcquisitionDefinition>
      : undefined,
  }
}

function validateImportedFieldValue(
  ...
  const error = () =>
    new Error(
      `Valor inválido para “${field.id}” na habilidade “${abilityId}”. Esperado: ${field.type}.`,
    )
  ...
  if (field.type === 'dice') {
    if (
      typeof value !== 'string' ||
      (field.allowedDice?.length && !field.allowedDice.includes(value as never))
    ) throw error()
    return value
  }
  if (
  ...
  if (field.type === 'boolean') {
    return (
      <label className="flex items-center gap-2 self-end rounded-lg border border-border px-3 py-2">
        ...
        />
        <span className="text-sm text-textH">{field.name}</span>
      </label>
    )
  }
  if (field.type === 'select') {
  ...
  if (field.type === 'multiSelect') {
    ...
    return (
      <fieldset className="rounded-lg border border-border p-3">
        <legend className="px-1 text-xs text-text">{field.name}</legend>
        {field.options.map((option) => (
          <label key={option.value} className="mr-3 inline-flex items-center gap-1 text-sm text-textH">
            <input
              type="checkbox"
...
function presetTitle(
  type: CustomAbilityTypeDefinition | undefined,
