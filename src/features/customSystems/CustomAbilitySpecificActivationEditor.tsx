...
export function CustomAbilitySpecificActivationEditor({
  definition,
  ability,
  onChange,
}: Props) {
  const activation = ability.activation
  const usageMode = !activation?.usage
    ? 'inherit'
    : (activation.usage.mode ?? 'limited')
  const resourceMode = activation?.resourceChanges === undefined
    ? 'inherit'
    : 'specific'

  function setActivation(next: CustomAbilityActivationDefinition | undefined) {
    onChange({ ...ability, activation: next })
  }

  function patchActivation(patch: Partial<CustomAbilityActivationDefinition>) {
  ...
  function setUsageMode(mode: 'inherit' | 'unlimited' | 'limited') {
    ...
    if (mode === 'inherit') {
      ...
      next.usage = { mode: 'unlimited', reset: 'manual' }
    } else {
      const current = activation?.usage
      next.usage = {
        mode: 'limited',
        reset: current?.reset ?? 'manual',
        maximum: current?.maximum,
        maximumFormula: current?.maximumFormula,
      }
    }
    setActivation(next)
  }
  ...
  function setResourceChanges(resourceChanges: CustomAbilityResourceChangeDefinition[]) {
    ...
    <section className="rounded-lg border border-border p-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-text">
            Por padrão, a habilidade herda as regras do tipo. Ative a configuração específica somente quando esta entrada do compêndio precisar se comportar de forma diferente.
          </p>
        </div>
      ...
      {activation ? (
        <div className="mt-4 grid gap-4">
          <div className="grid gap-3 md:grid-cols-2">
            <SelectField
              ...
              onChange={(kind) =>
                patchActivation({ kind: (kind || undefined) as AbilityKind | undefined })
              }
            />
            <SelectField
              label="Ação"
          ...
          <div className="rounded-lg border border-border bg-bg-subtle p-3">
            ...
            {usageMode === 'limited' ? (
              <FormulaField
                ...
                onChange={(maximumFormula) =>
                  patchUsage({ maximumFormula: maximumFormula || undefined })
                }
              />
            ) : null}
          </div>
              ...
              <div className="mt-3 grid gap-3">
                {(activation.resourceChanges ?? []).map((change, index) => (
                  <ResourceChangeRow
                    ...
                    onChange={(next) =>
                      setResourceChanges(
                        (activation.resourceChanges ?? []).map((entry, current) =>
        ...
        <div className="mt-3 rounded-lg border border-dashed border-border p-3 text-xs text-text">
          Herdando integralmente a ativação, categoria de ação, usos e efeitos de recurso configurados no tipo de habilidade.
        </div>
      )}
    </section>
  )
...
function ResourceChangeRow({
  ...
  const isCustom = change.target.source === 'customSystem'
  const targetValue = isCustom ? change.target.resourceId : change.target.resource
    ...
    <article className="rounded-lg border border-border p-3">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        <SelectField
          ...
          onChange={(operation) =>
            onChange({ ...change, operation: operation as CustomAbilityResourceChangeDefinition['operation'] })
          }
        />
        <SelectField
          label="Origem"
              ...
              target:
                source === 'native'
                  ...
                  : {
                      ...
                      resourceId: definition.resources[0]?.id ?? '',
                    },
            })
          }
        />
        <SelectField
          ...
          value={targetValue ?? ''}
          options={isCustom
            ? definition.resources.map((resource) => [resource.id, resource.name])
            : [...NATIVE_RESOURCES]}
          onChange={(resource) =>
            onChange({
              ...change,
              target: isCustom
                ...
                : {
                    source: 'native',
                    resource: resource as 'hitPoints' | 'temporaryHitPoints' | 'inspiration' | 'exhaustion',
                  },
            })
          }
        />
...
function FormulaField({
  ...
  return (
    <div className="mt-3 rounded-lg border border-accentBorder bg-accentBg/30 p-3">
      ...
      {value.trim() ? (
        <div className={`mt-2 text-xs ${error ? 'text-red-300' : 'text-emerald-300'}`}>
  ...
  return (
    <label className="grid gap-1">
      ...
      <select className="input-base" value={value} onChange={(event) => onChange(event.target.value)}>
        {options.map(([id, name]) => (
          <option key={id || 'inherit'} value={id}>{name}</option>
        ))}
      </select>
    </label>
  )
...
function newResourceChange(definition: CustomSystemDefinition): CustomAbilityResourceChangeDefinition {
  return definition.resources.length
    ...
    : {
        id: crypto.randomUUID(),
        target: { source: 'native', resource: 'hitPoints' },
        operation: 'spend',
        amount: 1,
      }
}
...
function normalizeActionKind(value: unknown): AbilityActionKind | undefined {
  ...
  return ACTION_KINDS.some(([kind]) => kind && kind === value)
