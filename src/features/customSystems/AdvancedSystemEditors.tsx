  ...
  return <MasterDetail title="Automações" description="Execute efeitos automaticamente quando eventos da ficha ou do combate acontecerem." items={draft.automations} selected={selected} onSelect={setSelected} onAdd={add} empty="Nenhuma automação criada.">
    {current ? <div className="grid gap-4">
      ...
      <Section title="Condições" description="Todas as condições precisam ser verdadeiras para a automação executar.">
        <ConditionsEditor conditions={current.conditions ?? []} draft={draft} onChange={(conditions) => replace({ ...current, conditions })} />
      </Section>

      <Section title="Efeitos" description="Os efeitos são executados na ordem exibida.">
        <EffectsEditor effects={current.effects} draft={draft} onChange={(effects) => replace({ ...current, effects })} />
      </Section>
...
function ConditionsEditor({ conditions, draft, onChange }: { conditions: CustomCondition[]; draft: CustomSystemDefinition; onChange: (conditions: CustomCondition[]) => void }) {
  function add() { onChange([...conditions, { left: { type: 'field', fieldId: draft.fields[0]?.id ?? '' }, operator: 'equals', right: { type: 'literal', value: true } }]) }
  ...
  return <div className="grid gap-3">
    ...
    {conditions.map((condition, index) => <article key={`condition-${index}`} className="rounded-lg border border-border p-3">
      <div className="grid gap-3 md:grid-cols-[1fr_200px_1fr_auto]">
        <OperandEditor label="Valor esquerdo" operand={condition.left} draft={draft} onChange={(left) => replace(index, { ...condition, left })} />
        ...
        {!['isTruthy','isFalsy'].includes(condition.operator) ? <OperandEditor label="Valor direito" operand={condition.right ?? { type: 'literal', value: true }} draft={draft} onChange={(right) => replace(index, { ...condition, right })} /> : <div />}
...
function OperandEditor({ label, operand, draft, onChange }: { label: string; operand: CustomOperand; draft: CustomSystemDefinition; onChange: (operand: CustomOperand) => void }) {
  return <div className="grid gap-2">
    <Select label={label} value={operand.type} options={['literal','field','resource','characterPath','formula']} labels={['Valor fixo','Campo','Recurso','Variável do personagem','Fórmula']} onChange={(type) => onChange(defaultOperand(type, draft))} />
    ...
    {operand.type === 'field' ? <ReferenceSelect label="Campo" value={operand.fieldId} options={draft.fields} onChange={(fieldId) => onChange({ ...operand, fieldId })} /> : null}
    {operand.type === 'resource' ? <><ReferenceSelect label="Recurso" value={operand.resourceId} options={draft.resources} onChange={(resourceId) => onChange({ ...operand, resourceId })} /><Select label="Propriedade" value={operand.property ?? 'current'} options={['current','maximum','temporary']} labels={['Atual','Máximo','Temporário']} onChange={(property) => onChange({ ...operand, property: property as any })} /></> : null}
    {operand.type === 'characterPath' ? <Input label="Caminho" value={operand.path} onChange={(path) => onChange({ ...operand, path })} /> : null}
    {operand.type === 'formula' ? <Input label="Fórmula" value={operand.formula} onChange={(formula) => onChange({ ...operand, formula })} /> : null}
  </div>
}

function EffectsEditor({ effects, draft, onChange }: { effects: CustomEffectDefinition[]; draft: CustomSystemDefinition; onChange: (effects: CustomEffectDefinition[]) => void }) {
  function add(type: CustomEffectDefinition['type']) {
    const effect: CustomEffectDefinition = type === 'modifyResource'
      ? { type, resourceId: draft.resources[0]?.id ?? '', operation: 'add', value: 1 }
      : type === 'setField'
        ? { type, fieldId: draft.fields[0]?.id ?? '', value: true }
        : { type, fieldId: draft.fields[0]?.id ?? '', operation: 'add', value: 1 }
  ...
  return <div className="grid gap-3">
    ...
    {effects.map((effect, index) => <article key={`effect-${index}`} className="rounded-lg border border-border p-3">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {effect.type === 'modifyResource' ? <ReferenceSelect label="Recurso" value={effect.resourceId} options={draft.resources} onChange={(resourceId) => replace(index, { ...effect, resourceId })} /> : <ReferenceSelect label="Campo" value={effect.fieldId} options={draft.fields} onChange={(fieldId) => replace(index, { ...effect, fieldId })} />}
        {effect.type !== 'setField' ? <Select label="Operação" value={effect.operation} options={OPERATIONS} labels={OPERATION_LABELS} onChange={(operation) => replace(index, { ...effect, operation: operation as CustomNumericOperation })} /> : null}
        <Input label="Valor" value={String(effect.value ?? '')} onChange={(value) => replace(index, { ...effect, value: optionalNumber(value), formula: undefined } as CustomEffectDefinition)} />
        <Input label="Ou fórmula" value={effect.formula ?? ''} onChange={(formula) => replace(index, { ...effect, formula: formula || undefined } as CustomEffectDefinition)} />
      </div>
    </article>)}
...
function defaultOperand(type: string, draft: CustomSystemDefinition): CustomOperand { if (type === 'field') return { type, fieldId: draft.fields[0]?.id ?? '' }; if (type === 'resource') return { type, resourceId: draft.resources[0]?.id ?? '', property: 'current' }; if (type === 'characterPath') return { type, path: 'character.level' }; if (type === 'formula') return { type, formula: '0' }; return { type: 'literal', value: true } }
