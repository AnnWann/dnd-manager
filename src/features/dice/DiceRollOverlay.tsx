    ...
    <aside className="fixed bottom-4 right-4 z-50 w-[min(92vw,340px)] rounded-xl border border-accentBorder bg-bg p-3 shadow-xl">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="truncate text-xs font-semibold text-textH">
            {latest.label}
          </div>
            ...
            className={
              latest.natural === 20
                ...
                : latest.natural === 1
                  ...
                  : "text-2xl font-black text-textH"
            }
          >
            {latest.total}
          </div>
          <div className="text-[10px] text-textMuted">
            {formatBreakdown(latest)}
          </div>
        </div>
      </div>
      ...
      <div className="mt-2 flex items-center justify-between border-t border-border pt-2">
        ...
        <button
          type="button"
          className="text-[10px] text-textMuted hover:text-textH"
          onClick={() => setHistory([])}
        >
          Limpar
        </button>
      ...
      {expanded ? (
        <div className="mt-2 grid max-h-48 gap-1 overflow-y-auto">
          {history.slice(1).map((roll) => (
            <div
              key={roll.id}
              className="flex items-center justify-between gap-2 rounded-md border border-border bg-bg-subtle px-2 py-1.5"
            >
              <span className="min-w-0 truncate text-[11px] text-text">
                {roll.label}
              </span>
              <span className="shrink-0 text-xs font-bold text-textH">
                {roll.total}
              </span>
            </div>
          ))}
          {history.length === 1 ? (
            <div className="text-[10px] text-textMuted">Nenhuma rolagem anterior.</div>
          ) : null}
        </div>
      ) : null}
    </aside>
