...
export function UserSpellsTab() {
  ...
  const [castingTimeFilter, setCastingTimeFilter] =
    useState<CastingTimeFilter>("all")
  ...
  const filteredSpells = useMemo(() => {
    ...
    const result = spells.filter((spell) => {
      ...
      const approvedCampaigns =
        record?.campaigns.filter((campaign) => campaign.status === "APPROVED") ?? []
      const characterLinked = Boolean(record?.characters?.length)
      ...
      const sourceMatches = (() => {
        ...
        if (sourceFilter === "campaign") return approvedCampaigns.length > 0
      ...
      const searchableText = normalizeSearch(
        [
          ...
          ...approvedCampaigns.map((campaign) => campaign.name),
          ...(record?.characters?.map((character) => character.name) ?? []),
        ]
          .filter(Boolean)
          .join(" "),
      )
    ...
    return result.toSorted((left, right) => {
      ...
      if (sortMode === "recent") {
        const leftRecord = recordByIndex.get(left.index)
        const rightRecord = recordByIndex.get(right.index)
        const leftDate = leftRecord?.updatedAt ?? ""
        const rightDate = rightRecord?.updatedAt ?? ""
  ...
  const ownedCount = records.filter((record) => record.ownedByCurrentUser).length
  const campaignCount = records.filter((record) =>
    record.campaigns.some((campaign) => campaign.status === "APPROVED"),
  ).length
  const characterCount = records.filter((record) => record.characters?.length).length
  ...
  function archiveSpell(spell: Spell) {
    ...
    const confirmed = window.confirm(
      `Arquivar “${spellName(spell)}”? A magia deixará de aparecer na biblioteca, mas o histórico será preservado.`,
    )
    ...
    <div className="grid gap-4">
      <section className="rounded-xl border border-border bg-bg p-4 shadow-theme-sm">
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-xs">
          ...
          <LibraryBadge label={`${characterCount} em personagens`} />
        </div>

        {loading ? (
          <p className="mt-3 text-xs text-textMuted">Carregando magias relacionais...</p>
        ) : null}
      ...
      <Card>
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-textMuted">
                {filteredSpells.length} de {spells.length} magias exibidas.
              </div>
            </div>
        ...
        <CardContent>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <Input
              value={query}
              placeholder="Nome, descrição, escola, campanha..."
            ...
            <FilterSelect value={sourceFilter} onChange={setSourceFilter}>
              ...
              <option value="owned">Minhas homebrew</option>
              <option value="campaign">Homebrew de campanha</option>
              <option value="character">Usadas em personagens</option>
            </FilterSelect>
          ...
          {advancedOpen ? (
            <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              <FilterSelect value={classFilter} onChange={setClassFilter}>
                ...
                {SPELL_CLASS_OPTIONS.map((entry) => (
                  <option key={entry.value} value={entry.value}>{entry.label}</option>
                ))}
              </FilterSelect>

              <FilterSelect value={concentrationFilter} onChange={setConcentrationFilter}>
                <option value="all">Concentração: qualquer</option>
                <option value="yes">Com concentração</option>
                <option value="no">Sem concentração</option>
              </FilterSelect>

              <FilterSelect value={ritualFilter} onChange={setRitualFilter}>
                <option value="all">Ritual: qualquer</option>
                <option value="yes">Apenas rituais</option>
                <option value="no">Sem ritual</option>
              </FilterSelect>

              <FilterSelect value={castingTimeFilter} onChange={setCastingTimeFilter}>
                <option value="all">Qualquer conjuração</option>
              ...
              <FilterSelect value={attackFilter} onChange={setAttackFilter}>
                <option value="all">Ataque: qualquer</option>
                <option value="yes">Exige ataque</option>
                <option value="no">Sem ataque</option>
              </FilterSelect>

              <FilterSelect value={saveFilter} onChange={setSaveFilter}>
                <option value="all">Salvaguarda: qualquer</option>
                <option value="yes">Exige salvaguarda</option>
                <option value="no">Sem salvaguarda</option>
              </FilterSelect>

              <FilterSelect value={sortMode} onChange={setSortMode}>
                <option value="level-name">Ordenar por nível</option>
                <option value="name">Ordenar por nome</option>
                <option value="recent">Homebrew mais recentes</option>
              </FilterSelect>
            </div>
          ) : null}
        </CardContent>
      ...
      <div className="grid gap-3">
        {filteredSpells.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border bg-bg p-6 text-center text-sm text-textMuted">
            ...
            <article key={spell.index} className="rounded-xl border border-border bg-bg p-4 shadow-theme-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <button type="button" className="min-w-0 flex-1 text-left" onClick={() => setViewingSpell(spell)}>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {!spell.homebrew ? (
                      ...
                      <LibraryBadge label="Homebrew compartilhada" />
                    )}
                  </div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-[10px]">
                    ...
                    {spell.classes.map((className) => (
                      <LibraryBadge key={className} label={CLASS_NAMES[className]} />
                    ))}
                    {approvedCampaigns.map((campaign) => (
                      <LibraryBadge key={campaign.id} label={campaign.name} />
                    ))}
                    {record?.characters?.map((character) => (
                      <LibraryBadge key={character.id} label={`Personagem: ${character.name}`} />
                    ))}
                  </div>

                  <p className="mt-3 line-clamp-3 whitespace-pre-wrap text-sm leading-6 text-text">
                ...
                <div className="flex shrink-0 flex-wrap gap-2">
                  ...
                  {owned ? (
                    <>
                      <Button size="sm" variant="secondary" onClick={() => openEdit(spell)}>Editar</Button>
                      <Button size="sm" variant="danger" onClick={() => archiveSpell(spell)}>Arquivar</Button>
                    </>
                  ) : null}
                </div>
              </div>
      ...
      {creatorOpen ? (
        <ModalFrame title={editingSpell ? "Editar magia" : "Criar magia homebrew"} onClose={closeCreator} maxWidth="max-w-3xl">
          <SpellCreatorModule
      ...
      {viewingSpell ? (
        <ModalFrame title={spellName(viewingSpell)} onClose={() => setViewingSpell(null)} maxWidth="max-w-3xl">
  ...
  return (
    <div className="grid gap-5 text-sm text-text">
      ...
      <section className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        <Info label="Tempo de conjuração" value={formatCastingTime(spell)} />
        <Info label="Alcance" value={formatRange(spell)} />
        <Info label="Duração" value={formatDuration(spell)} />
        <Info label="Componentes" value={formatComponents(spell)} />
        <Info label="Classes" value={spell.classes.map((entry) => CLASS_NAMES[entry]).join(", ") || "Nenhuma"} />
        <Info label="Alvo" value={formatTargeting(spell)} />
        <Info label="Área" value={formatArea(spell)} />
        <Info label="Rolagens" value={spell.rollMode.join(", ") || "Nenhuma"} />
        <Info label="Dano" value={spell.damageDice ? `${spell.damageDice.quantity}${spell.damageDice.sides}` : "Nenhum"} />
      </section>

      {spell.material?.trim() ? (
        <Info label="Material" value={spell.material} />
      ) : null}

      <section>
        <h3 className="font-semibold text-textH">Descrição</h3>
        <div className="mt-2 whitespace-pre-wrap leading-6">{spell.description?.trim() || "Sem descrição."}</div>
      </section>

      <section>
        <h3 className="font-semibold text-textH">Em níveis superiores</h3>
        <div className="mt-2 whitespace-pre-wrap leading-6">{spell.higherLevelText?.trim() || "Sem efeito adicional."}</div>
      </section>

      {spell.headcanon?.trim() ? (
        <section>
          <h3 className="font-semibold text-textH">Descrição personalizada</h3>
          <div className="mt-2 whitespace-pre-wrap leading-6">{spell.headcanon}</div>
        </section>
      ) : null}
    </div>
  )
...
function ModalFrame({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
      <div className={`max-h-[90vh] w-full ${maxWidth} overflow-y-auto rounded-2xl bg-bg shadow-xl`}>
...
function LibraryBadge({ label }: { label: string }) {
  return <span className="rounded-full border border-accentBorder bg-accentBg px-2.5 py-1 text-textH">{label}</span>
}
...
function matchesBoolean(filter: BooleanFilter, value: boolean): boolean {
...
function formatCastingTime(spell: Spell): string {
  ...
  const amount = spell.castingTime.value || 1
  if (spell.castingTime.type === "reaction" && spell.castingTime.reactionWhen) {
    return `${label}: ${spell.castingTime.reactionWhen}`
  }
  if (spell.castingTime.type === "special" && spell.castingTime.special) {
    return spell.castingTime.special
  }
  return amount === 1 ? label : `${amount} ${label.toLocaleLowerCase("pt-BR")}`
}
...
function formatRange(spell: Spell): string {
  ...
  const origin = originLabels[spell.range.origin] ?? spell.range.origin
  return spell.range.distance > 0 ? `${origin}, ${spell.range.distance} m` : origin
}
...
function formatDuration(spell: Spell): string {
  ...
  const label = unitLabels[spell.duration.unit] ?? spell.duration.unit
  if (spell.duration.value <= 0) return label
  return `${spell.duration.value} ${label}${spell.duration.value === 1 ? "" : "s"}`
}

function formatComponents(spell: Spell): string {
  if (!spell.components.length) return "Sem componentes"
  const components = spell.components.join(", ")
  return spell.material?.trim() ? `${components} (${spell.material})` : components
}

function formatTargeting(spell: Spell): string {
  const labels: Record<string, string> = {
    self: "Pessoal",
    "single-creature": "Uma criatura",
    "multiple-creatures": "Múltiplas criaturas",
    area: "Área",
    object: "Objeto",
    special: "Especial",
  }
  const base = labels[spell.targeting.kind] ?? spell.targeting.kind
  return spell.targeting.targetCount && spell.targeting.targetCount > 1
    ? `${base} (${spell.targeting.targetCount})`
    : base
}

function formatArea(spell: Spell): string {
  const area = spell.range.area
  if (!area && !spell.targeting.affectsArea) return "Nenhuma"
  const shape = area?.shape ?? spell.targeting.areaShape ?? "área"
  const size = area?.size ?? spell.targeting.areaSize
  const labels: Record<string, string> = {
    circle: "Círculo",
    square: "Quadrado",
    cone: "Cone",
  ...
  return size ? `${labels[shape] ?? shape}, ${size} m` : labels[shape] ?? shape
}
