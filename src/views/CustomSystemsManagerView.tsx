  ...
  return <div className="grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)]">
    <aside className="rounded-xl border border-border bg-bg p-3">
      ...
      <div className="mt-3 grid gap-2">
        {systems.definitions.map((definition) => <button key={definition.id} type="button" onClick={() => setSelectedId(definition.id)} className={`rounded-lg border px-3 py-2 text-left ${selectedId === definition.id ? 'border-accent bg-accentBg' : 'border-border hover:bg-accentBg'}`}>
    ...
    {!draft ? <Message title="Nenhum sistema selecionado">Selecione ou crie um sistema.</Message> : <SystemEditor draft={draft} setDraft={setDraft} error={error} onSave={saveSystem} onDelete={deleteSystem} onDuplicate={duplicateSystem} />}
  </div>
}
...
function SystemEditor({ draft, setDraft, error, onSave, onDelete, onDuplicate }: EditorProps) {
  ...
  return <section className="min-w-0 rounded-xl border border-border bg-bg">
    ...
    <div className="p-4">
      ...
      {tab === 'advanced' ? <AdvancedSystemEditors draft={draft} setDraft={setDraft} /> : null}
    </div>
  </section>
}
...
function OptionListEditor({ options, onChange }: { options: CustomSelectOption[]; onChange: (options: CustomSelectOption[]) => void }) {
  ...
  function addOption() { const label = `Opção ${options.length + 1}`; onChange([...options, { label, value: uniqueOptionValue(slugify(label), options) }]) }
  function updateLabel(index: number, label: string) { const current = options[index]; const follows = !current.value || current.value === slugify(current.label) || /^option-\d+$/.test(current.value); onChange(options.map((option, i) => i === index ? { ...option, label, value: follows ? uniqueOptionValue(slugify(label), options, index) : current.value } : option)) }
  function updateValue(index: number, value: string) { onChange(options.map((option, i) => i === index ? { ...option, value: slugify(value) } : option)) }
  return <section className="mt-3 rounded-lg border border-border p-3">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h4 className="text-sm font-medium text-textH">Opções</h4><p className="mt-1 text-xs text-text">Adicione os nomes que poderão ser escolhidos na ficha.</p></div><div className="flex gap-2"><button type="button" onClick={() => setAdvanced(!advanced)} className="rounded-lg border border-border px-3 py-2 text-xs text-text hover:bg-accentBg">{advanced ? 'Ocultar opções avançadas' : 'Opções avançadas'}</button><Button primary onClick={addOption}><Plus className="h-4 w-4" /> Adicionar opção</Button></div></div>
    <div className="mt-3 grid gap-2">{options.map((option, index) => <div key={`option-row-${index}`} className={`grid gap-2 rounded-lg border border-border p-2 ${advanced ? 'md:grid-cols-[1fr_1fr_auto]' : 'md:grid-cols-[1fr_auto]'}`}><Input label={`Opção ${index + 1}`} value={option.label} onChange={(label) => updateLabel(index, label)} />{advanced ? <Input label="Identificador interno" value={option.value} onChange={(value) => updateValue(index, value)} /> : null}<div className="flex items-end"><IconButton title="Remover opção" onClick={() => onChange(options.filter((_, i) => i !== index))}><Trash2 className="h-4 w-4" /></IconButton></div></div>)}{!options.length ? <div className="rounded-lg border border-dashed border-border p-4 text-center text-sm text-text">Nenhuma opção adicionada.</div> : null}</div>
  </section>
}
...
function FormulaEditor({ definition, formula, resultType, onChange }: { definition: CustomSystemDefinition; formula: string; resultType: 'number' | 'text' | 'boolean'; onChange: (formula: string, resultType: 'number' | 'text' | 'boolean') => void }) {
  ...
  return <div className="mt-3 rounded-lg border border-accentBorder bg-accentBg/30 p-3"><div className="grid gap-3 md:grid-cols-[180px_minmax(0,1fr)]"><Select label="Tipo do resultado" value={resultType} options={['number','text','boolean']} onChange={(value) => onChange(formula, value as typeof resultType)} /><Input label="Expressão" value={formula} onChange={(value) => onChange(value, resultType)} /></div><div className="mt-3"><FormulaVariablePicker variables={variables} onSelect={(path) => onChange(`${formula}${formula.trim() ? ' ' : ''}${path}`, resultType)} /></div><div className="mt-3 text-xs text-text">Funções: <code>min</code>, <code>max</code>, <code>round</code>, <code>floor</code>, <code>ceil</code>, <code>abs</code>, <code>clamp</code> e <code>if</code>.</div><div className={`mt-2 text-xs ${error ? 'text-red-300' : 'text-emerald-300'}`}>{error ?? 'Fórmula válida.'}</div></div>
}

function ResourcesEditor({ draft, setDraft }: Pick<EditorProps, 'draft' | 'setDraft'>) {
  return <Collection title="Recursos" onAdd={() => setDraft({ ...draft, resources: [...draft.resources, newResource()] })} empty="Nenhum recurso criado.">{draft.resources.map((resource, index) => <ResourceRow key={`resource-row-${index}`} definition={draft} resource={resource} onChange={(next) => setDraft({ ...draft, resources: draft.resources.map((entry, i) => i === index ? next : entry) })} onRemove={() => setDraft({ ...draft, resources: draft.resources.filter((_, i) => i !== index) })} />)}</Collection>
}
...
function ResourceRow({ definition, resource, onChange, onRemove }: { definition: CustomSystemDefinition; resource: CustomResourceDefinition; onChange: (resource: CustomResourceDefinition) => void; onRemove: () => void }) {
  ...
  const variables = listCustomFormulaVariables(definition)
  return <article className="rounded-lg border border-border p-3"><div className="grid gap-3 md:grid-cols-4"><Input label="Nome" value={resource.name} onChange={(name) => onChange({ ...resource, name })} /><Input label="ID" value={resource.id} onChange={(id) => onChange({ ...resource, id: slugify(id) })} /><Select label="Tipo" value={resource.type} options={['number','checkboxes','dicePool','charges']} onChange={(value) => onChange({ ...resource, type: value as CustomResourceDefinition['type'] })} /><Select label="Permissão" value={resource.editPermission ?? 'ownerAndMaster'} options={['ownerAndMaster','owner','masterOnly','automaticOnly']} onChange={(value) => onChange({ ...resource, editPermission: value as CustomResourceDefinition['editPermission'] })} /><Input label="Mínimo" type="number" value={String(resource.minimum ?? '')} onChange={(value) => onChange({ ...resource, minimum: optionalNumber(value) })} /><Input label="Máximo fixo" type="number" value={String(resource.maximum ?? '')} onChange={(value) => onChange({ ...resource, maximum: optionalNumber(value) })} /><Input label="Valor inicial" type="number" value={String(resource.initialValue ?? '')} onChange={(value) => onChange({ ...resource, initialValue: optionalNumber(value) })} /></div><div className="mt-3 rounded-lg border border-border p-3"><Input label="Fórmula do máximo" value={formula} onChange={(value) => onChange({ ...resource, maximumFormula: value || undefined })} /><div className="mt-2"><FormulaVariablePicker variables={variables} onSelect={(path) => onChange({ ...resource, maximumFormula: `${formula}${formula.trim() ? ' ' : ''}${path}` })} /></div>{formula ? <div className={`mt-2 text-xs ${validateCustomFormula(formula, definition) ? 'text-red-300' : 'text-emerald-300'}`}>{validateCustomFormula(formula, definition) ?? 'Fórmula válida.'}</div> : null}</div><div className="mt-3 flex flex-wrap items-center justify-between gap-3"><div className="flex gap-4 text-xs text-text"><Check label="Ajuste manual" checked={Boolean(resource.allowManualAdjustment)} onChange={(checked) => onChange({ ...resource, allowManualAdjustment: checked })} /><Check label="Temporário" checked={Boolean(resource.allowTemporaryValue)} onChange={(checked) => onChange({ ...resource, allowTemporaryValue: checked })} /></div><Button danger onClick={onRemove}><Trash2 className="h-4 w-4" /> Remover</Button></div></article>
}

function Collection({ title, onAdd, empty, children }: { title: string; onAdd: () => void; empty: string; children: ReactNode }) { const count = Array.isArray(children) ? children.length : 1; return <div><div className="mb-3 flex items-center justify-between"><h3 className="font-medium text-textH">{title}</h3><Button primary onClick={onAdd}><Plus className="h-4 w-4" /> Adicionar</Button></div><div className="grid gap-3">{count ? children : <div className="rounded-lg border border-dashed border-border p-6 text-center text-sm text-text">{empty}</div>}</div></div> }
...
function convertFieldType(field: CustomFieldDefinition, type: CustomFieldDefinition['type']): CustomFieldDefinition { const base = { id: field.id, name: field.name, description: field.description, required: field.required, editPermission: field.editPermission }; if (type === 'number') return { ...base, type }; if (type === 'boolean') return { ...base, type }; if (type === 'select' || type === 'multiSelect') return { ...base, type, options: [] }; if (type === 'dice') return { ...base, type }; if (type === 'reference') return { ...base, type, target: 'character' }; if (type === 'formula') return { ...base, type, formula: '', resultType: 'number', editPermission: 'automaticOnly' }; return { ...base, type } }
function validateDefinition(definition: CustomSystemDefinition): string { if (!definition.id.trim()) return 'O sistema precisa de um ID.'; if (!definition.name.trim()) return 'O sistema precisa de um nome.'; const ids = [...definition.fields.map((entry) => entry.id), ...definition.resources.map((entry) => entry.id)]; if (ids.some((id) => !id.trim())) return 'Campos e recursos precisam de IDs.'; if (ids.length !== new Set(ids).size) return 'IDs de campos e recursos não podem se repetir.'; for (const field of definition.fields) { if (field.type === 'select' || field.type === 'multiSelect') { if (field.options.some((option) => !option.label.trim() || !option.value.trim())) return `Todas as opções de “${field.name}” precisam de nome.`; const values = field.options.map((option) => option.value); if (values.length !== new Set(values).size) return `Os identificadores das opções de “${field.name}” não podem se repetir.` } if (field.type === 'formula') { const formulaError = validateCustomFormula(field.formula, definition); if (formulaError) return `Fórmula de “${field.name}”: ${formulaError}` } } for (const resource of definition.resources) { if (!resource.maximumFormula) continue; const formulaError = validateCustomFormula(resource.maximumFormula, definition); if (formulaError) return `Máximo de “${resource.name}”: ${formulaError}` } return '' }
function uniqueOptionValue(base: string, options: CustomSelectOption[], ignoredIndex = -1): string { const normalized = base || 'opcao'; const used = new Set(options.filter((_, index) => index !== ignoredIndex).map((option) => option.value)); if (!used.has(normalized)) return normalized; let suffix = 2; while (used.has(`${normalized}-${suffix}`)) suffix += 1; return `${normalized}-${suffix}` }
