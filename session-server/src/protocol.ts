...
export type SessionHitDicePool = {
  current: number;
  max: number;
};
...
/** Rest remains one semantic/log event even when it mutates multiple domains. */
...
export type SessionRestReverseOperation = {
  ...
  snapshot: {
    hp: SessionHpState;
    // Future resources/conditions/supplies are added here, not as extra log events.
  };
};
...
const DIE_SIDES = new Set<SessionDieSides>([
...
export function parseClientSessionMessage(raw: string): ClientSessionMessage | null {
  ...
  if (value.type === "session.heartbeat" && typeof value.clientId === "string" && value.clientId.length > 0) {
  ...
  if (value.type === "session.hp.operation" && isAuthoritativeOperation(value.operation)) {
...
function isHpSeed(value: unknown): value is SessionHpSeed {
  ...
  return typeof value.characterId === "string" && value.characterId.length > 0 &&
    ...
    isFiniteNumber(value.current) && isFiniteNumber(value.temporary) &&
    isFiniteNumber(value.max) && isFiniteNumber(value.currentMax) &&
    isFiniteNumber(value.maxHpBonus) && isHitDiceState(value.hitDice);
}
...
function isAuthoritativeOperation(value: unknown): value is SessionAuthoritativeOperation {
  ...
  switch (value.type) {
    ...
    case "character.hitDice.add":
      return isDieSide(value.side) && isFiniteNumber(value.amount);
    case "character.hitDice.remove":
      return isDieSide(value.side);
    case "character.rest.short":
      return isFiniteNumber(value.healing) && isHitDiceConsumption(value.hitDiceConsumption);
...
function isHitDiceState(value: unknown): value is SessionHitDiceState {
  if (!isRecord(value)) return false;
  return Object.entries(value).every(([side, pool]) =>
    isDieSide(side) && isRecord(pool) && isFiniteNumber(pool.current) && isFiniteNumber(pool.max),
  );
}
...
function isDieSide(value: unknown): value is SessionDieSides {
  return typeof value === "string" && DIE_SIDES.has(value as SessionDieSides);
}
