...
export class SessionActor extends RaceSessionActor {
  ...
  private async handleOperation(webSocket: WebSocket, connection: SessionConnection, operation: SessionProfileOperation): Promise<void> {
    ...
    if (!profile) {
      sendError(webSocket, "PROFILE_OPERATION_INVALID", "The requested profile state is invalid.");
      return;
    }
    const next = character.with("profile", profile);
    if (JSON.stringify(character.get("profile")) === JSON.stringify(next.get("profile"))) return;
    ...
    if (canCoalesce) {
      ...
      log.push({
        ...
        operation: operation as unknown as UnifiedLogRecord["operation"],
        reverseOperation: {
          type: "session.profile.restore", characterId: operation.characterId, snapshot: structuredClone(stored),
  ...
  private async tryProfileUndo(webSocket: WebSocket, logId: string): Promise<boolean> {
    ...
    log.push({
      ...
      reverseOperation: {
        type: "session.profile.restore", characterId: reverse.characterId, snapshot: structuredClone(current),
      } as unknown as UnifiedLogRecord["reverseOperation"],
    });
    const nextLog = log.slice(-MAX_HP_LOG_RECORDS);
