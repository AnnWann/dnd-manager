...
The remaining custom-system runtime surfaces are system actions and automations, standard-action overrides and native-stat overrides. These should reuse the same runtime-config access boundary and aggregate reverse strategy when one semantic action touches several authoritative domains.

The current character-workspace compatibility adapter infers custom-ability activation from the legacy local updater when activation produces observable side effects. A later cleanup can let the custom-system UI dispatch `character.customSystem.ability.activate` directly, removing that inference path while retaining the same server contract.
